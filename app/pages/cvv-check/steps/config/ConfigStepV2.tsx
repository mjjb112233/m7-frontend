...
import { Channel, DetectionMode, DetectionConfig, DetectionModeType } from "@/app/shared/types"

// 简化的LoadingState组件
const LoadingState = ({ message, description }: { message: string; description: string }) => (
...
export function ConfigStepV2({
    ...
    const fetchConfig = async () => {
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          ...
          console.log("[ConfigStepV2] 使用模拟配置数据")
          setDetectionConfig(mockConfig as DetectionConfig)
        ...
        if (result && result.success && result.data) {
          console.log("[ConfigStepV2] 配置获取成功:", result.data)
          setDetectionConfig(result.data as DetectionConfig)
        }
      } catch (error) {
  ...
  const getChannelsForMode = (mode: DetectionMode): Channel[] => {
    ...
    const modeEntry = detectionConfig.detectionModes.find((item: any) => 
      ...
      <CardContent className="relative space-y-6 p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 justify-center">
            {detectionConfig?.detectionModes?.map((modeData: any) => {
              // 直接使用当前模式数据
